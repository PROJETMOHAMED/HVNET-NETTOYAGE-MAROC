...
if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
	...
	if( ini_get('allow_url_fopen') ) {
		...
		$verifyResponse = file_get_contents('www.google.com/recaptcha/api/siteverify?secret='.$secret.'&response='.$_POST['g-recaptcha-response']);
	    ...
	    $verifyResponse = curl_init("www.google.com/recaptcha/api/siteverify");
	...
	if($responseData->success) {
		...
		// Step 1 - Enter your email Addresse below.
		...
		foreach($_POST as $label => $value) {
			if( $label != 'g-recaptcha-response' ) {
				...
				// Use the commented code below to change label texts. On this example will change "Email" to "Email Addresse"
		...
		try {
			...
			$mail->AddAddresse($email);	 						       // Add another recipient

			//$mail->AddAddresse('person2@domain.com', 'Person 2');     // Add a secondary recipient
			//$mail->AddCC('person3@domain.com', 'Person 3');          // Add a "Cc" Addresse. 
			//$mail->AddBCC('person4@domain.com', 'Person 4');         // Add a "Bcc" Addresse. 
