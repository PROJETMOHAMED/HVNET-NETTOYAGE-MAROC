<?php
  ...
  * @version 1.0.6
  * @license The MIT License opensource.org/licenses/mit-license.php
  * @link  f6design.com/journal/2013/06/20/tweetphp-display-tweets-on-your-website-using-php/
  * 
  * Notes:
  ...
  * 
  * Credits:
  * Feed parsing: github.com/themattharris/tmhOAuth
  * Hashtag/username parsing: github.com/mikenz/twitter-text-php
  */
 class TweetPHP {
    private $tmhOAuth;
    private $options;
    ...
    public function  __construct ($options = array()) {
      ...
      array(
          ...
          'date_format'           => '%I:%M %p %b %e%O', // The defult date format e.g. 12:08 PM Jun 12th. See: php.net/manual/en/function.strftime.php
          'date_lang'             => null, // Language for date e.g. 'fr_FR'. See: php.net/manual/en/function.setlocale.php
      ...
      if (!$this->tweet_found) {
        ...
        $this->tweet_list = $this->options['twitter_wrap_open'] . $this->options['tweet_wrap_open'] . $this->options['error_message'] . ' ' . $this->options['meta_wrap_open'] .'<a href="twitter.com/' . $this->options['twitter_screen_name'] . '">' . $this->options['error_link_text'] . '</a>' . $this->options['meta_wrap_close'] . $this->options['tweet_wrap_close'] . $this->options['twitter_wrap_close'];
    ...
    private function parse_tweet ($tweet) {
      ...
      $href = 'twitter.com/' . $tweet['user']['screen_name'] . '/status/' . $tweet['id_str'];
